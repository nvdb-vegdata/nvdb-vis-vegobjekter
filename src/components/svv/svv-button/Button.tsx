...
import { SVVSpinner } from '../svv-spinner'
import type { ButtonType } from './Button.types'
import { useButtonTestId } from './test-parts'

type ButtonProps<T extends ElementType> = ButtonType<T> & Omit<ComponentPropsWithRef<T>, keyof ButtonType<T>>
...
const SVVButtonInner = <T extends ElementType = 'button'>(props: ButtonProps<T>, ref?: ForwardedRef<any>) => {
  const { children, loading, icon, iconPlacement = 'left', loadingString, size = 'md', color = 'primary', as, testId, ...restProps } = props as ButtonProps<T>
  ...
  return (
    ...
    >
      {icon && iconPlacement === 'left' && (!loading || children) && isValidElement(icon) && cloneElement(icon, { 'aria-hidden': true })}
      {children}
      {icon && iconPlacement === 'right' && (!loading || children) && isValidElement(icon) && cloneElement(icon, { 'aria-hidden': true })}
      {loading && <SVVSpinner inline size="small" aria-label={loadingString} />}
    </Component>
  )
}

export const SVVButton = forwardRef(SVVButtonInner) as ForwardRefExoticComponent<ButtonProps<any> & RefAttributes<any>>
