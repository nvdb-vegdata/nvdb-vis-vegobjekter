...
import * as ts from "typescript"; // flyttet over config
 ...
 */
function collectNodes<T extends TSESTree.Node>(
  node: TSESTree.Node,
...
function expressionReferencesTestId(expr: TSESTree.Expression | TSESTree.PrivateIdentifier): boolean {
  if (expr.type === "Identifier") {
    return expr.name === TESTID_PROP_NAME ||
           expr.name === "testIds" ||
           /TestId$/.test(expr.name);
  }
  ...
  if (expr.type === "CallExpression") {
    ...
    return expr.arguments.some(arg =>
      arg.type === "Identifier" && arg.name === TESTID_PROP_NAME
    );
  }
...
function typeHasTestId(typeNode: any, aliasMap: Map<string, any>, importResolver?: (name: string) => boolean): boolean {
  ...
  function check(node: any): boolean {
    ...
    switch (node.type) {
      ...
      case "TSTypeLiteral":
        return (node.members || []).some((m: any) =>
          m.type === "TSPropertySignature" && m.key?.name === TESTID_PROP_NAME
        );
      default:
        return false;
    }
...
function returnsJSX(fn: TSESTree.FunctionDeclaration | TSESTree.ArrowFunctionExpression): boolean {
  ...
  const jsxNodes = collectNodes(fn.body, (n): n is TSESTree.JSXElement | TSESTree.JSXFragment =>
...
>({
  ...
  create(context, [opts]) {
    ...
    function tsTypeHasTestId(esNode: TSESTree.Node | undefined): boolean {
      ...
      } catch { /* ignore type resolution errors */ return false; }
    }
    ...
    const forwardPropNames = (opts.forwardPropNames && opts.forwardPropNames.length > 0)
      ? opts.forwardPropNames
      : [TESTID_PROP_NAME];
    if (!fileOk) return {};
    ...
    function importResolver(typeName: string): boolean {
      ...
      if (!filePath) { importedCache.set(typeName, false); return false; }
      try {
    ...
    function paramHasTestIdExtended(param: TSESTree.Parameter, localAliasMap: Map<string, any>): boolean {
      if (param.type === "ObjectPattern") {
        const hasProperty = param.properties.some(p =>
          p.type === "Property" &&
          p.key.type === "Identifier" &&
          p.key.name === TESTID_PROP_NAME
        );
